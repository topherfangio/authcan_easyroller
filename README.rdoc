...
Since this project is a basis for other projects, the easiest way to get started is to simply fork
it to your new project and begin work. Alternatively, you can click the "Download Source" button at
the top if you intend to use a source control other than git (or if you're just not into the whole
forking thing). However, you do have to have the following gems installed (<tt>sudo gem install ...</tt>):
...
The file +app/models/user.rb+ stores the roles mask of all available roles within the system. If
you change the roles mask, you will almost certainly need to change the abilities file.

A user's abilities are defined in +app/models/ability.rb+. I generally prefer to specify what
...
You can also define your own abilities if they don't tie to a k
...
Note that you must pass a nil object as the second argument to +can?+ and you must specify all object
types when defining the ability. I'll get in touch with the developer to see if this can be a tad bit
more streamlined.
...
Using +can?+ and +cannot?+ is the preferred method of checking authorization privileges, however,
...
don't have to. That is the exact purpose of the +Ability+ class! Use it to your advantage and
